  ...
  factory: (require) => {
    ...
    function GatewayPanel(props) {
      ...
      const scanUrl = (() => {
        ...
        // 外网模式下开启 autoLogin，或者在局域网开启了 lanAuth 时，自动附带 ?key= 免密直通
        const needsToken = (qrMode === 'remote' && autoLogin) || (qrMode === 'lan' && lanAuth);
        if (needsToken && curPw) {
      ...
      function renderQrTab() {
        ...
        return h('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } },
          ...
          h('div', {
            ...
            }, WifiIcon(15), lanAuth ? '📶 局域网直连 (密码防护)' : '📶 局域网直连 (免密)')
          ),
          ...
          Boolean(baseTargetUrl) && h('div', {
            ...
            h('div', {
              ...
              h('div', {
                ...
                ? (autoLogin ? '📱 扫码免密进入' : '📱 扫码输入口令')
                : (lanAuth ? (curPw ? '📱 扫码免密进入' : '📱 扫码输入口令') : '📶 局域网免密'))
            ),
            ...
            h('div', {
              ...
              h('div', {
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 } },
                  ...
                  h('span', {
                    ...
                    ? (autoLogin ? '免密直达' : '需输入口令')
                    : (lanAuth ? (curPw ? '密码防护 · 免密直达' : '需验证口令') : '无需口令')
                  )
                ),
                ...
                }, autoLogin ? '免密: 开' : '免密: 关') : h('button', {
                  type: 'button',
                  className: 'dsh-gw-btn',
                  disabled: busy === 'lanAuth',
                  onClick: () => handleToggleLanAuth(!lanAuth),
                  title: lanAuth ? '点击关闭局域网密码保护（恢复纯免密）' : '点击开启局域网密码保护',
                  style: {
                    background: lanAuth ? '#0284c7' : '#ffffff',
                    border: lanAuth ? 'none' : '1px solid #cbd5e1',
                    color: lanAuth ? '#ffffff' : '#64748b',
                    borderRadius: 14,
                    padding: '3px 9px',
                    fontSize: 11,
                    fontWeight: 600,
                    boxShadow: lanAuth ? '0 1px 4px rgba(2, 132, 199, 0.25)' : 'none',
                  },
                }, busy === 'lanAuth' ? '切换中…' : (lanAuth ? '密码保护: 开' : '密码保护: 关'))
              ),
              ...
              }, copiedUrl ? [CheckIcon(14), '已复制到剪贴板'] : [CopyIcon(14), ((qrMode === 'remote' && autoLogin) || (qrMode === 'lan' && lanAuth)) ? '复制免密直达链接' : '复制连接地址']),

              // 4. 次级按钮行 (打开浏览器 & 复制原地址)
              h('div', { style: { display: 'flex', gap: 6 } },
                ((qrMode === 'remote' && autoLogin) || (qrMode === 'lan' && lanAuth)) && h('button', {
                  type: 'button',
                  className: 'dsh-gw-btn',
                  title: '复制不带密码的原地址',
                ...
                h('button', {
                  ...
                  style: {
                    flex: ((qrMode === 'remote' && autoLogin) || (qrMode === 'lan' && lanAuth)) ? 1 : 'none',
                    width: ((qrMode === 'remote' && autoLogin) || (qrMode === 'lan' && lanAuth)) ? undefined : '100%',
                    background: '#ffffff',
                    border: '1px solid #cbd5e1',
                    color: '#334155',
                    borderRadius: 7,
              ...
              // 5. 底部紧凑口令条 (公网模式，或局域网开启了密码保护时展示)
              (qrMode === 'remote' || lanAuth) && h('div', {
                style: {
                  background: '#f0f9ff',
                  border: '1px solid #bae6fd',
                  borderRadius: 8,
          ...
          qrMode === 'lan' && h('div', {
            style: {
              width: '100%',
              fontSize: 12,
              color: lanAuth ? '#0284c7' : '#64748b',
              textAlign: 'center',
              lineHeight: 1.4,
            },
          }, lanAuth ? '🔒 局域网已开启口令防护，当前二维码已自动内嵌免密凭证，手机扫码秒进。' : '💡 同一 WiFi 下直接扫码即可进入 DSH Web，无需输入任何口令。')
        );
      }
      ...
      function renderSettingsTab() {
        ...
        return h('div', { style: { display: 'flex', flexDirection: 'column', gap: 16 } },
          ...
          h('div', {
            ...
            h('div', { style: { fontSize: 12, color: '#64748b', lineHeight: 1.4 } },
              lanAuth
                ? '🔒 局域网保护已生效：同内网/WiFi设备访问网关需输入口令（扫码可通过内嵌 Token 秒进）。'
          ...
          h('div', {
            ...
            h('div', { style: { fontSize: 12, color: '#64748b', lineHeight: 1.5 } },
              (data && data.gateway && data.gateway.port3080Exposed)
                ? '⚠️ 严重安全警告：检测到 DSH 3080 端口已被局域网或外部网络直接侦听！请确保仅将 3081 网关对外开放，严禁直接映射 DSH 3080 端口，否则外部请求可能绕过网关鉴权！'
            ...
            showLogs && h('div', {
              ...
              auditLogs.length === 0
                ...
                : auditLogs.slice(0, 20).map((l, idx) => {
                  const isFail = l.status && (l.status.includes('FAIL') || l.status.includes('RATE_LIMIT') || l.status.includes('DENIED'));
                  return h('div', {
                    key: idx,
                    style: {
                      display: 'flex',
