      ...
      function renderQrTab() {
        ...
        return h('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } },
          ...
          Boolean(baseTargetUrl) && h('div', {
            ...
            h('div', {
              ...
              h('div', {
                ...
                ? (curPw ? '扫码直接登录' : '扫码验证口令')
                : (lanAuth ? (curPw ? '扫码直接登录' : '扫码验证口令') : '局域网免密访问'))
            ),
            ...
            h('div', {
              ...
              }, copiedUrl ? [CheckIcon(14), '已复制到剪贴板'] : [CopyIcon(14), (curPw && (qrMode === 'remote' || lanAuth)) ? '复制扫码直接登录链接' : '复制连接地址']),

              // 4. 次级按钮行 (打开浏览器 & 复制原地址)
              h('div', { style: { display: 'flex', gap: 6 } },
                (curPw && (qrMode === 'remote' || lanAuth)) && h('button', {
                  type: 'button',
                  className: 'dsh-gw-btn',
                  title: '复制不带凭证的原始域名地址（需手动输入口令）',
                ...
                h('button', {
                  ...
                  style: {
                    flex: (curPw && (qrMode === 'remote' || lanAuth)) ? 1 : 'none',
                    width: (curPw && (qrMode === 'remote' || lanAuth)) ? undefined : '100%',
                    background: '#ffffff',
                    border: '1px solid #cbd5e1',
                    color: '#334155',
                    borderRadius: 7,
              ...
              h('div', {
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 } },
                  ...
                  h('span', { style: { fontSize: 11, color: '#0284c7', fontWeight: 600, flexShrink: 0 } }, '访问口令:'),
                  h('span', {
                    style: {
                      fontFamily: 'monospace',
                      fontSize: 12,
          ...
          h('div', {
            ...
            ? '🔒 公网访问强制口令验证，当前二维码已内嵌安全凭证，手机扫码无需手动输入。'
            : (lanAuth ? '🔒 局域网已开启口令防护，当前二维码已内嵌安全凭证，手机扫码秒进。' : '💡 同一 WiFi 下设备在浏览器直接打开对应 IP 即可免密访问。')
          )
        );
      }
