    ...
    function GatewayPanel(props) {
      ...
      function renderQrTab() {
        ...
        return h('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } },
          ...
          // 【新增神级特性】扫码免密自动登录开关 (仅在公网模式下显示)
          qrMode === 'remote' && Boolean(wanUrl) && h('div', {
            style: {
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
              background: autoLogin ? '#f0f9ff' : '#f8fafc',
              border: autoLogin ? '1px solid #bae6fd' : '1px solid #e2e8f0',
              borderRadius: 12,
              padding: '9px 14px',
              gap: 12,
              transition: 'all 0.2s',
            ...
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 } },
              h('span', { style: { color: autoLogin ? '#0284c7' : '#94a3b8', display: 'flex' } }, BoltIcon(16)),
              h('div', { style: { minWidth: 0 } },
                h('div', { style: { fontSize: 13, fontWeight: 600, color: autoLogin ? '#0369a1' : '#475569' } },
                  '扫码免密直达'
                ),
                h('div', { style: { fontSize: 11, color: autoLogin ? '#0284c7' : '#94a3b8' } },
                  autoLogin ? '✓ 二维码已内嵌安全凭证，手机扫码免输口令直接进桌面' : '未带凭证，手机扫码后需手动输入访问口令'
                )
              )
            ),
            h('button', {
              ...
              style: {
                ...
                fontSize: 12,
                fontWeight: 600,
                boxShadow: autoLogin ? '0 2px 6px rgba(2, 132, 199, 0.25)' : 'none',
              },
            }, autoLogin ? '已开启' : '未开启')
          ),
          ...
          Boolean(baseTargetUrl) && h('div', {
            ...
            h('div', {
              style: {
                ...
                padding: 14,
                border: '1px solid #e2e8f0',
                boxShadow: '0 8px 24px -4px rgba(2, 132, 199, 0.12), 0 2px 6px rgba(0, 0, 0, 0.04)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                minWidth: 208,
            ...
            h('div', {
              ...
              h('button', {
                type: 'button',
                className: 'dsh-gw-btn',
                title: autoLogin && qrMode === 'remote' ? '复制免密直通链接' : '复制链接',
                onClick: () => copyToClipboard(scanUrl, () => {
                  setCopiedUrl(true);
                  setTimeout(() => setCopiedUrl(false), 2000);
                }),
                style: {
                  background: copiedUrl ? '#ecfdf5' : '#f0f9ff',
                  border: copiedUrl ? '1px solid #a7f3d0' : '1px solid #bae6fd',
                  color: copiedUrl ? '#059669' : '#0284c7',
                  borderRadius: 8,
                  padding: '5px 10px',
                  fontSize: 12,
                  fontWeight: 600,
                },
              }, copiedUrl ? [CheckIcon(13), '已复制'] : [CopyIcon(13), autoLogin && qrMode === 'remote' ? '复制免密链接' : '复制链接']),
              autoLogin && qrMode === 'remote' && h('button', {
                ...
                style: {
                  background: copiedRawUrl ? '#ecfdf5' : '#f8fafc',
                  border: copiedRawUrl ? '1px solid #a7f3d0' : '1px solid #e2e8f0',
                  color: copiedRawUrl ? '#059669' : '#64748b',
                  borderRadius: 8,
                  padding: '5px 8px',
                  fontSize: 11,
                },
            ...
            qrMode === 'remote' && h('div', {
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 } },
                h('span', { style: { color: '#0284c7', display: 'flex' } }, ShieldKeyIcon(16)),
                h('div', { style: { minWidth: 0 } },
                  h('div', { style: { fontSize: 11, color: '#0284c7', fontWeight: 600 } }, '外网访问口令:'),
                  h('div', {
                    style: {
                      fontFamily: 'monospace',
                      fontSize: 13,
